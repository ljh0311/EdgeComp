...
class BabyMonitor:
    ...
    def __init__(self, camera_width=1280, camera_height=720):
        ...
        self.frame_lock = (
            threading.Lock()
        )  # Prevents frame corruption during multi-threading
        ...
        self.motion_threshold = (
            100  # Pixel distance for rapid movement (lower = more sensitive)
        )
        self.fall_threshold = (
            1.5  # Aspect ratio change for fall detection (lower = more sensitive)
        )
    ...
    def setup_ui(self):
        ...
        self.alert_container = ttk.Frame(
            self.main_container, style="AlertContainer.TFrame"
        )
        ...
        self.dismiss_button = ttk.Button(
            self.alert_frame, text="Dismiss Alert", command=self.dismiss_alert
        )
        self.dismiss_button.grid(row=1, column=0, pady=(0, 5))

        # Alert history section
        self.alert_history_container = ttk.Frame(
            self.alert_container, style="AlertHistory.TFrame"
        )
        self.alert_history_container.grid(row=1, column=0, sticky="nsew", pady=(5, 0))

        # History title
        self.history_title = ttk.Label(
            self.alert_history_container,
            text="Alert History",
            font=("Arial", int(initial_height * 0.02), "bold"),
            foreground="white",
            background="#1a1a1a",
        ...
        self.alert_canvas = tk.Canvas(
            self.alert_history_container, background="#1a1a1a", highlightthickness=0
        )
        self.alert_canvas.grid(row=1, column=0, sticky="nsew", padx=(10, 0))

        # Add scrollbar
        self.alert_scrollbar = ttk.Scrollbar(
            self.alert_history_container,
            orient=tk.VERTICAL,
        ...
        self.alert_history_frame = ttk.Frame(
            self.alert_canvas, style="AlertHistory.TFrame"
        )
        self.alert_canvas.create_window(
            (0, 0),
            ...
            for result in results:
                ...
                for box in boxes:
                    ...
                    if box.cls == 0:  # person class
                        ...
                        if (
                            height > width * 1.5
                        ):  # Person is significantly taller than wide
                            position = "standing"
                        elif (
                            width > height * 1.5
                        ):  # Person is significantly wider than tall
                            position = "lying down"
                        ...
                        self.logger.info(
                            f"Person {person_count} detected in {position} position with confidence {conf:.2f}"
                        )

            # Display total person count in top-left corner
            cv2.putText(
                frame,
                f"People in room: {person_count}",
                (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.7,
                (255, 255, 255),
            ...
            try:
                ...
                if ret and frame is not None:
                    ...
                    detection_thread = threading.Thread(
                        target=self._process_detection, args=(process_frame,)
                    )
    ...
    def _process_detection(self, frame):
        ...
        try:
            ...
            if self.previous_frame is None:
                ...
                frame = self.detect_motion_and_falls(
                    frame, self.person_detector(frame)[0].boxes
                )
                self.previous_frame = frame.copy()
        except Exception as e:
    ...
    def detect_motion_and_falls(self, frame, boxes):
        ...
        try:
                ...
                for i, box in enumerate(boxes):
                    if box.cls == 0:  # person class
                        ...
                        if i in self.previous_positions:
                            ...
                            previous_ratio = (
                                prev_width / prev_height if prev_height != 0 else 0
                            )
                            ...
                            if previous_ratio != 0:
                                ...
                                if (
                                    ...
                                    or current_ratio
                                    < previous_ratio / self.fall_threshold
                                ):
                                    fall_detected = True
                                    cv2.putText(
                                        frame,
                                    ...
                                    self.logger.warning(
                                        f"Potential fall detected for person {i+1}"
                                    )
                            ...
                            if movement > self.motion_threshold:
                                ...
                                self.logger.warning(
                                    f"Rapid movement detected for person {i+1}"
                                )
            ...
            if fall_detected:
                self.show_alert(
                    "FALL DETECTED! Check on person immediately!", "critical"
                )
            elif rapid_motion:
    ...
    def show_alert(self, message, level="warning"):
        ...
        self.alert_label.configure(
            text=full_message, background=bg_color, font=("Arial", font_size, "bold")
        )
        self.alert_frame.pack(
            fill=tk.X, pady=(0, 5)
        )  # Ensure the alert frame is packed
        ...
        self.current_alert_timer = self.root.after(
            self.alert_duration, self.hide_main_alert
        )
    ...
    def add_to_alert_history(self, message, level):
        ...
        history_label.pack(
            ...
            pady=2,
            before=(
                self.alert_history_frame.winfo_children()[0]
...
def main():
    ...
    if IS_RASPBERRY_PI:
        try:
            ...
            print(
                "Please install picamera2 for Raspberry Pi: sudo apt install -y python3-picamera2"
            )
            return
