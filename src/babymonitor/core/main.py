...
class BabyMonitorSystem:
                ...
                try:
                    ...
                    if not only_web:
                        ...
                        if hasattr(self, 'emotion_recognizer'):
                            ...
                            self.emotion_status.configure(text="😊  Emotion: Not Available")
                except Exception as e:
    ...
    def setup_ui(self):
        ...
        # Camera selection
        self.camera_select = ttk.Combobox(camera_controls, state="readonly", style="Dark.TCombobox", width=30)
        self.camera_select.pack(side=tk.LEFT, padx=5)
        self.camera_select.bind("<<ComboboxSelected>>", self.on_camera_selected)

        # Resolution selection
        self.resolution_select = ttk.Combobox(camera_controls, state="readonly", style="Dark.TCombobox", width=15)
        ...
        self.camera_btn = ttk.Button(camera_controls, text="Toggle Camera", command=self.toggle_camera)
                ...
                if hasattr(self, "person_detector"):
                    try:
                        ...
                        if not self.only_web and hasattr(self, 'detection_status'):
                            self.detection_status.configure(text=f"👀  Detection: {len(detections)} people")
                    except Exception as e:
                        ...
                        if canvas_width > 1 and canvas_height > 1:
                            ...
                            def update_canvas(photo):
                                ...
                                try:
                                    ...
                                    self.logger.error(f"Error updating canvas: {str(e)}")
    ...
    def update_audio_visualization(self, audio_data):
        ...
        try:
            ...
            if hasattr(self, 'waveform_data'):
                ...
                self.waveform_data = alpha * audio_data + (1 - alpha) * self.waveform_data
            ...
            if hasattr(self, 'decibel_label'):
                self.decibel_label.configure(
                    text=f"Sound Level: {db:.1f} dB",
                    foreground=self.get_db_color(db)
                )
            ...
            if selected_resolution and isinstance(selected_resolution, str):
                if self.camera.set_resolution(selected_resolution):
                    ...
                    if current_resolution != selected_resolution:
                        self.resolution_select.set(current_resolution)
                        self.logger.warning(f"Camera set to {current_resolution} instead of requested {selected_resolution}")
                else:
        ...
        for component in reversed(self.initialized_components):
            try:
                if component == 'web_app':
                    ...
                    if hasattr(self, 'emotion_recognizer') and self.emotion_recognizer is not None:
                        self.emotion_recognizer.stop()
...
class DevWindow:
    def __init__(self, master):
        ...
        # Audio metrics
        self.audio_metrics = tk.Text(self.audio_frame, height=10, bg=bg_darker, fg=text_color)
        ...
        # Detection metrics
        self.detection_metrics = tk.Text(self.detection_frame, height=10, bg=bg_darker, fg=text_color)
        ...
        # Emotion metrics
        self.emotion_metrics = tk.Text(self.emotion_frame, height=10, bg=bg_darker, fg=text_color)
    ...
    def update_detection_visualization(self, frame, detections):
        ...
        self.detection_canvas.create_image(canvas_width//2, canvas_height//2, 
    ...
    def update_emotion_visualization(self, emotions):
        ...
        for emotion, confidence in emotions.items():
            # Draw bar background
            canvas.create_rectangle(0, y, width, y + bar_height, 
                                 fill="#1e1e1e", outline="#333333")
            
            # Draw confidence bar
            bar_width = int(width * confidence)
            canvas.create_rectangle(0, y, bar_width, y + bar_height, 
                                 fill="#4CAF50", outline="")
            
            # Draw label
            canvas.create_text(10, y + bar_height//2, 
...
def main():
    ...
    parser.add_argument("--dev", action="store_true", help="Run in developer mode with local GUI and debug window")
