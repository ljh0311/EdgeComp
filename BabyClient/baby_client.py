...
Options:
    ...
    --mode MODE             Connection mode: auto (try MQTT first), mqtt (MQTT only), or http (HTTP/Socket.IO only) [default: auto]
"""
...
try:
    ...
    print(
        "Error: PyQt5 is required. Please install it using: pip install PyQt5 PyQtWebEngine"
    )
    sys.exit(1)
...
try:
    ...
    print(
        "Error: Required libraries missing. Please install them using: pip install python-socketio paho-mqtt"
    )
    sys.exit(1)
...
class MQTTClient(QObject):
    ...
    def __init__(self, broker_host, broker_port=1883):
        ...
        # Use the latest API version for MQTT client
    ...
    def on_connect(self, client, userdata, flags, rc, properties=None):
        if rc == 0:
            ...
            error_msg = f"Failed to connect to MQTT broker with code: {rc}"
            print(error_msg)
    ...
    def connect_to_broker(self):
        try:
            # Validate the host (attempt to resolve it)
            import socket

            try:
                socket.gethostbyname(self.broker_host)
            except socket.gaierror:
                error_msg = f"Invalid host: '{self.broker_host}' cannot be resolved"
                print(error_msg)
                self.connection_error.emit(error_msg)
                return False
...
class VideoThread(QThread):
    ...
    connection_status = pyqtSignal(bool)  # New signal for connection status
    ...
    def add_alert(self, level, message):
        ...
        if level == "warning":
            alert_frame.setStyleSheet(
                "background-color: rgba(255, 193, 7, 0.2); border: 1px solid #ffc107;"
            )
        elif level == "danger":
            alert_frame.setStyleSheet(
                "background-color: rgba(220, 53, 69, 0.2); border: 1px solid #dc3545;"
            )
        elif level == "info":
            alert_frame.setStyleSheet(
                "background-color: rgba(13, 202, 240, 0.2); border: 1px solid #0dcaf0;"
            )
        else:
            alert_frame.setStyleSheet(
                "background-color: rgba(108, 117, 125, 0.2); border: 1px solid #6c757d;"
            )
...
class BabyMonitorClient(QMainWindow):
    ...
    def __init__(self, server_url, mqtt_host, mqtt_port, connection_mode="auto"):
        super().__init__()
        ...
        self.update_status_signal.connect(self.update_status_message)

        # Initialize MQTT client
        self.mqtt_client = MQTTClient(self.mqtt_host, self.mqtt_port)
        ...
        self.mqtt_client.connection_error.connect(self.handle_video_error)
        ...
        self.video_thread.connection_status.connect(self.update_camera_status)
        ...
        # Connect to server after a short delay to allow UI to initialize
    ...
    def setup_ui(self):
        ...
        self.camera_status_label.setAlignment(Qt.AlignRight)
    ...
    def connect_to_server(self):
        ...
        self.update_status_message("Connecting to server...")

        if self.connection_mode == "mqtt":
            # MQTT only mode
            self._try_mqtt_connection()
        elif self.connection_mode == "http":
            # HTTP/Socket.IO only mode
            self._try_http_connection()
        else:
            # Auto mode - try MQTT first, then HTTP
            if not self._try_mqtt_connection():
                self._try_http_connection()

    def _try_mqtt_connection(self):
        """Try to connect using MQTT."""
        if self.connection_mode == "http":
            return False

        self.update_status_message(
            f"Trying MQTT connection to {self.mqtt_host}:{self.mqtt_port}..."
        )
        # Initialize MQTT client
        self.mqtt_client = MQTTClient(self.mqtt_host, self.mqtt_port)
        self.mqtt_client.emotion_update.connect(self.update_emotion_display)
        ...
        return self.mqtt_client.connect_to_broker()

    def _try_http_connection(self):
        """Try to connect using HTTP/Socket.IO."""
        if self.connection_mode == "mqtt":
            return False

        self.update_status_message(f"Trying HTTP connection to {self.server_url}...")
        try:
    ...
    def handle_mqtt_connection(self, connected):
        ...
        if connected:
            self.update_status_message("Connected to server via MQTT")
            self.update_connection_status(True)
        else:
            self.update_status_message("Disconnected from MQTT")
            self.update_connection_status(False)
            # Only try HTTP if we're in auto mode and MQTT failed
            if self.connection_mode == "auto":
                QTimer.singleShot(100, self._try_http_connection)
    ...
    def handle_socketio_connection(self, connected):
        ...
        if not connected and self.connection_mode == "http":
            ...
            QTimer.singleShot(100, self._try_mqtt_connection)
    ...
    def show_settings(self):
        ...
        host, ok1 = QInputDialog.getText(
            self, "Server Settings", "Server Host:", QLineEdit.Normal, self.mqtt_host
        )

        if ok1:
            port, ok2 = QInputDialog.getInt(
                self, "Server Settings", "Server Port:", self.mqtt_port, 1, 65535
            )

            if ok2:
                http_port, ok3 = QInputDialog.getInt(
                    self, "Server Settings", "HTTP Port:", 5000, 1, 65535
                )
    ...
    def update_status_message(self, message):
        """Thread-safe update of status bar message"""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="localhost",
        help="Host for the Baby Monitor Server (HTTP)",
    )
    parser.add_argument(
        "--port", type=int, default=5000, help="Port for the Baby Monitor Server (HTTP)"
    )
    parser.add_argument(
        "--mqtt-host", type=str, default="localhost", help="Host for the MQTT broker"
    )
    parser.add_argument(
        "--mqtt-port", type=int, default=1883, help="Port for the MQTT broker"
    )
    parser.add_argument(
        "--mode",
        type=str,
        choices=["auto", "mqtt", "http"],
        default="auto",
        help="Connection mode: auto (try MQTT first), mqtt (MQTT only), or http (HTTP/Socket.IO only)",
    )
    return parser.parse_args()


def main():
    """Main function to launch the Baby Monitor Client."""
    args = parse_args()
